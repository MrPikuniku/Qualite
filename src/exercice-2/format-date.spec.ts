...
describe(("Test de la fonction formatDate()"), function(){
    ...
    test("avec inclusion de l’heure et des minutes", function(){
        expect(formatDate(lundi)).toEqual(`${lundi.toLocaleDateString('fr-FR')} à ${lundi.getHours()}:${lundi.getMinutes()}` );
    ...
    test("test avec h comme séparateur", function(){
        expect(formatDate(lundi)).toEqual(`${lundi.toLocaleDateString('fr-FR')} à ${lundi.getHours()}h${lundi.getMinutes()}`);
